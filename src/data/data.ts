import { ExperienceData, NavData, ProjectsData, SkillsData } from '@/types';

export const navData: NavData = [
  {
    title: 'Home',
    link: '#home',
  },
  {
    title: 'About',
    link: '#about',
  },
  {
    title: 'Experience',
    link: '#experience',
  },
  {
    title: 'Skills',
    link: '#skills',
  },
  {
    title: 'Projects',
    link: '#projects',
  },
  {
    title: 'Contact',
    link: '#contact',
  },
];

export const experienceData: ExperienceData = [
  {
    role: 'Software Engineer Intern',
    company: 'Research Foundation of CUNY - Climastry',
    location: 'New York, NY',
    mode: 'Remote',
    time: 'Jun. 2024 - Aug. 2024',
    description:
      'Technologies Used: TypeScript, React, Redux, AWS (S3, Lambda, API Gateway), GitHub Actions',
    img: '/rfcuny.webp',
  },
  {
    role: 'Software Engineer Intern',
    company: 'SPEAKHIRE',
    location: 'New York, NY',
    mode: 'Remote',
    time: 'Feb. 2024 - Apr. 2024',
    description:
      'Technologies Used: TypeScript, React, Redux (RTK Query), Material UI, react-i18next Framework',
    img: '/speakhire.webp',
  },
  {
    role: 'Software Engineer Fellow',
    company: 'NYC Tech Talent Pipeline',
    location: 'Staten Island, NY',
    mode: 'Hybrid',
    time: 'Jun. 2023 - Aug. 2023',
    description:
      'Technologies Used: JavaScript, React, PostgreSQL, Node.js, Express.js, Jest, AWS (EC2)',
    img: '/ttp.webp',
  },
];

export const projectsData: ProjectsData = [
  {
    title: 'PickUpNYC',
    description:
      'A platform for soccer enthusiasts to connect, organize, and participate in pickup games.',
    link: 'https://github.com/pickupnyc/pickupnyc',
    demo: 'https://pickupnyc.onrender.com/',
    img: '/pickupnyc.webp',
    tech: ['/react.svg', '/javascript.svg', '/node.svg', 'postgresql.svg'],
    type: 'web',
  },
  {
    title: 'Spelling Bee App',
    description:
      'A multilingual study assistant for expanding vocabulary with randomly generated games.',
    link: 'https://github.com/PatrickLisiecki/spelling-bee-app',
    demo: 'https://github.com/PatrickLisiecki/spelling-bee-app',
    img: '/spelling-bee-app.webp',
    tech: ['/react.svg', '/typescript.svg', '/node.svg', 'sqlite.svg'],
    type: 'web',
  },
  {
    title: 'Flex Fusion',
    description:
      'A fitness application designed to provide users with a personalized fitness experience.',
    link: 'https://github.com/PatrickLisiecki/flex-fusion',
    demo: 'https://flexfusion.xyz/',
    img: '/flex-fusion.webp',
    tech: ['/react.svg', '/tailwind.svg', '/node.svg', 'postgresql.svg', 'aws.svg'],
    type: 'web',
  },
  {
    title: 'Flashcard Creator',
    description:
      'A user-friendly and interactive application for effective learning with digital flashcards.',
    link: 'https://github.com/PatrickLisiecki/flashcard-creator',
    demo: '',
    img: '/flashcard-creator.webp',
    tech: ['/typescript.svg', '/react.svg', 'node.svg', 'mongodb.svg'],
    type: 'web',
  },
  {
    title: 'Portfolio Website',
    description:
      'An interactive portfolio website for a developer built with Next.js and Framer Motion.',
    link: 'https://github.com/PatrickLisiecki/portfolio',
    demo: 'https://portfolio-patricklisiecki.vercel.app/',
    img: '/dev-portfolio.webp',
    tech: ['next.svg', '/typescript.svg', 'react.svg'],
    type: 'web',
  },
  {
    title: 'Novel Opinions',
    description: 'A functional dynamic book forum.',
    link: 'https://github.com/PatrickLisiecki/book-review-forum',
    demo: '',
    img: '/novel-opinions.webp',
    tech: ['/react.svg', '/tailwind.svg'],
    type: 'web',
  },
];

export const skillsData: SkillsData = [
  {
    title: 'TypeScript',
    type: 'Programming Language',
    img: '/typescript.svg',
    link: 'https://www.typescriptlang.org/',
  },
  {
    title: 'React',
    type: 'JavaScript Library',
    img: '/react.svg',
    link: 'https://react.dev/',
  },
  {
    title: 'Amazon Web Services',
    type: 'Cloud Computing',
    img: '/aws.svg',
    link: 'https://aws.amazon.com/',
  },
  {
    title: 'Python',
    type: 'Programming Language',
    img: '/python.svg',
    link: 'https://www.python.org/',
  },
  {
    title: 'Node.js',
    type: 'JavaScript Runtime',
    img: '/node.svg',
    link: 'https://nodejs.org/',
  },
  {
    title: 'Express.js',
    type: 'Web Framework',
    img: '/express.svg',
    link: 'https://expressjs.com/',
  },
  {
    title: 'PostgreSQL',
    type: 'Database',
    img: '/postgresql.svg',
    link: 'https://www.postgresql.org/',
  },
  {
    title: 'SQLite',
    type: 'Database',
    img: 'sqlite.svg',
    link: 'https://www.sqlite.org/',
  },
  {
    title: 'MongoDB',
    type: 'NoSQL Database',
    img: '/mongodb.svg',
    link: 'https://www.mongodb.com/',
  },
  {
    title: 'Docker',
    type: 'Container Development',
    img: '/docker.svg',
    link: 'https://www.docker.com/',
  },
  {
    title: 'Tailwind',
    type: 'CSS Framework',
    img: '/tailwind.svg',
    link: 'https://tailwindcss.com/',
  },
  {
    title: 'Git',
    type: 'Version Control',
    img: '/git.svg',
    link: 'https://git-scm.com/',
  },
];
